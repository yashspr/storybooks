...
! function (e, t) {
	console.log('image.min.js loaded');
	...
	_e = _e && _e.hasOwnProperty("default") ? _e["default"] : _e, Object.assign(_e.POPUP_TEMPLATES, {
		...
		function n() {
			...
			if (e || (e = P()), e) {
				...
				d.hasClass("fr-uploading") ? O() : g.popups.show("image.edit", a, i, t.outerHeight(), !0)
			}
		}
		...
		function A() {
			for (var e, t = "IMG" == g.el.tagName ? [g.el] : g.el.querySelectorAll("img"), a = 0; a < t.length; a++) {
				var i = c(t[a]);
				!g.opts.htmlUntouched && g.opts.useClasses ? ((g.opts.imageDefaultAlign || g.opts.imageDefaultDisplay) && (0 < (e = i).parents(".fr-img-caption").length && (e = e.parents(".fr-img-caption").first()), e.hasClass("fr-dii") || e.hasClass("fr-dib") || (e.addClass("fr-fi" + ue(e)[0]), e.addClass("fr-di" + he(e)[0]), e.css("margin", ""), e.css("float", ""), e.css("display", ""), e.css("z-index", ""), e.css("position", ""), e.css("overflow", ""), e.css("vertical-align", ""))), g.opts.imageTextNear || (0 < i.parents(".fr-img-caption").length ? i.parents(".fr-img-caption").first().removeClass("fr-dii").addClass("fr-dib") : i.removeClass("fr-dii").addClass("fr-dib"))) : g.opts.htmlUntouched || g.opts.useClasses || (g.opts.imageDefaultAlign || g.opts.imageDefaultDisplay) && C(i), g.opts.iframe && i.on("load", g.size.syncIframe)
			}
		}
		...
		function E(e) {
			...
			if (o)
				for (t = 0; t < o.length; t++) i.indexOf(o[t].getAttribute("src")) < 0 && g.events.trigger("image.removed", [c(o[t])]);
			if (o && e) {
				var s = [];
				for (t = 0; t < o.length; t++) s.push(o[t].getAttribute("src"));
				for (t = 0; t < a.length; t++) s.indexOf(a[t].getAttribute("src")) < 0 && g.events.trigger("image.loaded", [c(a[t])])
			}
			o = a
		}
		...
		function S() {
			if (l || function s() {
				...
				g.shared.$image_resizer ? (l = g.shared.$image_resizer, p = g.shared.$img_overlay, g.events.on("destroy", function () {
					...
					e.stopPropagation()
				}, !0), g.opts.imageResize && (l.append(R("nw") + R("ne") + R("sw") + R("se")), g.shared.$img_overlay = c(document.createElement("div")).attr("class", "fr-image-overlay"), p = g.shared.$img_overlay, e = l.get(0).ownerDocument, c(e).find("body").first().append(p)));
				g.events.on("shared.destroy", function () {
					...
					d && !d.hasClass("fr-uploading") ? pe(!0) : d && (S(), ve(), O(!1))
				});
				if (g.opts.imageResize) {
					e = l.get(0).ownerDocument, g.events.$on(l, g._mousedown, ".fr-handler", x), g.events.$on(c(e), g._mousemove, $), g.events.$on(c(e.defaultView || e.parentWindow), g._mouseup, U), g.events.$on(p, "mouseleave", U);
					var i = 1,
						r = null,
						n = 0;
					g.events.on("keydown", function (e) {
						if (d) {
							var t = -1 != navigator.userAgent.indexOf("Mac OS X") ? e.metaKey : e.ctrlKey,
								a = e.which;
							(a !== r || 200 < e.timeStamp - n) && (i = 1), (a == _e.KEYCODE.EQUALS || g.browser.mozilla && a == _e.KEYCODE.FF_EQUALS) && t && !e.altKey ? i = ee.call(this, e, 1, 1, i) : (a == _e.KEYCODE.HYPHEN || g.browser.mozilla && a == _e.KEYCODE.FF_HYPHEN) && t && !e.altKey ? i = ee.call(this, e, 2, -1, i) : g.keys.ctrlKey(e) || a != _e.KEYCODE.ENTER || (d.before("<br>"), T(d)), r = a, n = e.timeStamp
						}
		...
		function D(e) {
			Ce() ? d.parents(".fr-img-caption").css("width", e) : d.css("width", e)
		}
		...
		function x(e) {
			...
			D(n), p.show(), g.popups.hideAll(), ce()
		}
		...
		function $(e) {
			...
			if (f && d) {
				...
				if ((f.hasClass("fr-hnw") || f.hasClass("fr-hsw")) && (i = 0 - i), g.opts.imageResizeWithPercent) {
					...
					r = ((r + i) / c(n).outerWidth() * 100).toFixed(2), g.opts.imageRoundPercent && (r = Math.round(r)), D(r + "%"), (t = Ce() ? (g.helpers.getPX(d.parents(".fr-img-caption").css("width")) / c(n).outerWidth() * 100).toFixed(2) : (g.helpers.getPX(d.css("width")) / c(n).outerWidth() * 100).toFixed(2)) === r || g.opts.imageRoundPercent || D(t + "%"), d.css("height", "").removeAttr("height")
				} else r + i >= g.opts.imageMinWidth && (D(r + i), t = Ce() ? g.helpers.getPX(d.parents(".fr-img-caption").css("width")) : g.helpers.getPX(d.css("width"))), t !== r + i && D(t), ((d.attr("style") || "").match(/(^height:)|(; *height:)/) || d.attr("height")) && (d.css("height", f.data("start-height") * d.width() / f.data("start-width")), d.removeAttr("height"));
		...
		function U(e) {
			...
			if (f && d) {
				...
				f = null, p.hide(), S(), n(), g.undo.saveStep(), g.events.trigger("image.resizeEnd", [ye()])
			}
		}

		function I(e, t, a) {
			g.edit.on(), d && d.addClass("fr-error"), r[e] ? B(r[e]) : B(g.language.translate("Something went wrong. Please try again.")), !d && a && te(a), g.events.trigger("image.error", [{
				code: e,
				message: r[e]
			}, t, a])
		}
		...
		function O(e) {
			...
			if (t || (t = X()), t.find(".fr-layer.fr-active").removeClass("fr-active").addClass("fr-pactive"), t.find(".fr-image-progress-bar-layer").addClass("fr-active"), t.find(".fr-buttons").hide(), d) {
				var a = we();
		...
		function Y(e, t, a) {
			var i, r = c(document.createElement("img")).attr("src", e);
			if (t && void 0 !== t)
				for (i in t) t.hasOwnProperty(i) && "link" != i && (" data-" + i + '="' + t[i] + '"', r.attr("data-str" + i, t[i]));
			var n = g.opts.imageDefaultWidth;
			n && "auto" != n && (n = g.opts.imageResizeWithPercent ? "100%" : n + "px"), r.attr("style", n ? "width: " + n + ";" : ""), me(r, g.opts.imageDefaultDisplay, g.opts.imageDefaultAlign), r.on("load", a), r.on("error", a), g.edit.on(), g.events.focus(!0), g.selection.restore(), g.undo.saveStep(), g.opts.imageSplitHTML ? g.markers.split() : g.markers.insert(), g.html.wrap();
		...
		function G(e, t) {
			if (void 0 !== e && 0 < e.length) {
				...
				var a, i = e[0];
				if ((null === g.opts.imageUploadURL || g.opts.imageUploadURL == s) && !g.opts.imageUploadToS3) return function o(r, n) {
						...
						function l(t, a, e, r) {
							function n() {
								...
								e.off("load"), e.addClass("fr-uploading"), e.next().is("br") && e.next().remove(), g.placeholder.refresh(), T(e), S(), O(), g.edit.off(), t.onload = function () {
									M.call(t, e)
								}, t.onerror = z, t.upload.onprogress = K, t.onabort = W, c(e.off("abortUpload")).on("abortUpload", function () {
									4 != t.readyState && (t.abort(), r ? (r.attr("src", r.data("fr-old-src")), r.removeClass("fr-uploading")) : e.remove(), pe(!0))
								}), t.send(a)
							}
							var s = new FileReader;
							s.onload = function () {
								...
								r ? (r.get(0).addEventListener("load", n), r.on("error", function () {
									n(), c(this).off("error")
								}), g.edit.on(), g.undo.saveStep(), r.data("fr-old-src", r.attr("src")), r.attr("src", e)) : Y(e, null, n)
							}, s.readAsDataURL(e)
		...
		function H(e) {
			...
			if (t && t.files && t.files.length) {
				...
				if (a && a.type && -1 !== a.type.indexOf("image") && 0 <= g.opts.imageAllowedTypes.indexOf(a.type.replace(/image\//g, ""))) {
					...
					return g.popups.show("image.insert", r, n), O(), 0 <= g.opts.imageAllowedTypes.indexOf(a.type.replace(/image\//g, "")) ? (pe(!0), G(t.files)) : I(b), e.preventDefault(), e.stopPropagation(), !1
				}
			}
		}
		...
		function X(e) {
			...
			return 1 <= g.opts.imageInsertButtons.length && (a = g.popups.create("image.insert", f)), g.$wp && g.events.$on(g.$wp, "scroll", function () {
				d && g.popups.isVisible("image.insert") && ve()
			}),
				function p(i) {
					g.events.$on(i, "dragover dragenter", ".fr-image-upload-layer", function () {
						return c(this).addClass("fr-drop"), !1
					}, !0), g.events.$on(i, "dragleave dragend", ".fr-image-upload-layer", function () {
						...
						if (this.files) {
							...
							e.events.disableBlur(), i.find("input:focus").blur(), e.events.enableBlur(), e.image.upload(this.files, d)
						}
						c(this).val("")
					}, !0)
				}(a), a
		...
		function q(e) {
			...
			return g.$wp && g.events.$on(g.$wp, "scroll.image-alt", function () {
				d && g.popups.isVisible("image.alt") && j()
			}), a
		}
		...
		function te(e) {
			(e = e || we()) && !1 !== g.events.trigger("image.beforeRemove", [e]) && (g.popups.hideAll(), be(), pe(!0), g.undo.canDo() || g.undo.saveStep(), e.get(0) == g.el ? e.removeAttr("src") : (e.get(0).parentNode && "A" == e.get(0).parentNode.tagName ? (g.selection.setBefore(e.get(0).parentNode) || g.selection.setAfter(e.get(0).parentNode) || e.parent().after(_e.MARKERS), c(e.get(0).parentNode).remove()) : (g.selection.setBefore(e.get(0)) || g.selection.setAfter(e.get(0)) || e.after(_e.MARKERS), e.remove()), g.html.fillEmptyBlocks(), g.selection.restore()), g.undo.saveStep())
		}

		function ae(e) {
			var t = e.which;
			if (d && (t == _e.KEYCODE.BACKSPACE || t == _e.KEYCODE.DELETE)) return e.preventDefault(), e.stopPropagation(), te(), !1;
			if (d && t == _e.KEYCODE.ESC) {
				var a = d;
				return pe(!0), g.selection.setAfter(a.get(0)), g.selection.restore(), e.preventDefault(), !1
			}
			if (!d || t != _e.KEYCODE.ARROW_LEFT && t != _e.KEYCODE.ARROW_RIGHT) return d && t === _e.KEYCODE.TAB ? (e.preventDefault(), e.stopPropagation(), pe(!0), !1) : d && t != _e.KEYCODE.F10 && !g.keys.isBrowserAction(e) ? (e.preventDefault(), e.stopPropagation(), !1) : void 0;
			var i = d.get(0);
		...
		function ie(e) {
			if (e && "IMG" == e.tagName) {
				...
				for (var i = e.querySelectorAll("img.fr-uploading, img.fr-error, img.fr-draggable"), r = 0; r < i.length; r++) ie(i[r])
		}

		function re(e) {
			if (!1 === g.events.trigger("image.beforePasteUpload", [e])) return !1;
			d = c(e), S(), n(), ve(), O(), d.on("load", function () {
				S(), c(g.popups.get("image.insert").get(0)).find("div.fr-active.fr-error").length < 1 && O(), c(this).off("load")
			});
			for (var t = c(e).attr("src").split(","), a = atob(t[1]), i = [], r = 0; r < a.length; r++) i.push(a.charCodeAt(r));
			G([new Blob([new Uint8Array(i)], {
		...
		function ne() {
			g.opts.imagePaste ? g.$el.find("img[data-fr-image-pasted]").each(function (e, i) {
				if (g.opts.imagePasteProcess) {
					...
					t && "auto" != t && (t += g.opts.imageResizeWithPercent ? "%" : "px"), c(i).css("width", t).removeClass("fr-dii fr-dib fr-fir fr-fil"), me(c(i), g.opts.imageDefaultDisplay, g.opts.imageDefaultAlign)
				}
				if (0 === i.src.indexOf("data:")) re(i);
				...
				} else 0 !== i.src.indexOf("http") || 0 === i.src.indexOf("https://mail.google.com/mail") ? (g.selection.save(), c(i).remove(), g.selection.restore()) : c(i).removeAttr("data-fr-image-pasted")
		...
		function fe(e) {
			...
			g.toolbar.disable(), e && (e.stopPropagation(), e.preventDefault()), g.helpers.isMobile() && (g.events.disableBlur(), g.$el.blur(), g.events.enableBlur()), g.opts.iframe && g.size.syncIframe(), d = c(this), be(), S(), n(), g.browser.msie ? (g.popups.areVisible() && g.events.disableBlur(), g.win.getSelection && (g.win.getSelection().removeAllRanges(), g.win.getSelection().addRange(g.doc.createRange()))) : g.selection.clear(), g.helpers.isIOS() && (g.events.disableBlur(), g.$el.blur()), g.button.bulkRefresh(), g.events.trigger("video.hideResizer")
		}

		function pe(e) {
			d && (function t() {
				return ge
			}() || !0 === e) && (g.toolbar.enable(), l.removeClass("fr-active"), g.popups.hide("image.edit"), d = null, ce(), f = null, p && p.hide())
		}
		r[i] = "Image cannot be loaded from the passed link.", r[m] = "No link in upload response.", r[u] = "Error during file upload.", r[h] = "Parsing response failed.", r[v] = "File is too large.", r[b] = "Image file type is invalid.", r[7] = "Files can be uploaded only to same domain in IE 8 and IE 9.";
		var ge = !(r[8] = "Image file is corrupted.");
		...
		function we() {
			return Ce() ? d.parents(".fr-img-caption").first() : d
		}
		...
		return {
			_init: function Ae() {
				...
				(function e() {
					g.events.$on(g.$el, g._mousedown, "IMG" == g.el.tagName ? null : 'img:not([contenteditable="false"])', function (e) {
						...
						if (e.shiftKey && "" === g.selection.text().replace(/\n/g, "") && g.keys.isArrow(e.which)) {
							...
							t && "IMG" == t.tagName ? T(c(t)) : a && "IMG" == a.tagName && T(c(a))
						}
					}, !0), g.events.on("drop", H), g.events.on("element.beforeDrop", V), g.events.on("mousedown window.mousedown", de), g.events.on("window.touchmove", ce), g.events.on("mouseup window.mouseup", function () {
						if (d) return pe(), !1;
						ce()
						...
						d && (be(), g.selection.clear())
					}), g.events.on("image.resizeEnd", function () {
						g.win.getSelection && T(d)
						...
						g.node.isElement(t) && (g.opts.enter === _e.ENTER_BR ? e.after("<br>") : c(g.node.blockParent(e.get(0))).after("<" + g.html.defaultTag() + "><br></" + g.html.defaultTag() + ">"))
					})
					...
					if (d && e.which == _e.KEYCODE.ENTER) return !1
				}, !0), g.events.$on(g.$el, "keydown", function () {
					var e = g.selection.element();
					(e.nodeType === Node.TEXT_NODE || "BR" == e.tagName && g.node.isLastSibling(e)) && (e = e.parentNode), g.node.hasClass(e, "fr-inner") || (g.node.hasClass(e, "fr-img-caption") || (e = c(e).parents(".fr-img-caption").get(0)), g.node.hasClass(e, "fr-img-caption") && (c(e).after(_e.INVISIBLE_SPACE + _e.MARKERS), g.selection.restore()))
					...
					if (d && g.popups.isVisible("image.edit") && !g.popups.get("image.edit").find(":focus").length) {
						var t = we();
						Ce() ? (t.before(_e.START_MARKER), t.after(_e.END_MARKER), g.selection.restore(), g.paste.saveCopiedText(t.get(0).outerHTML, t.text())) : (be(), g.paste.saveCopiedText(d.get(0).outerHTML, d.attr("alt"))), "copy" == e.type ? setTimeout(function () {
							T(d)
					...
					var t = e.which;
					if (d && t == _e.KEYCODE.BACKSPACE) return e.preventDefault(), !1
				}), g.events.$on(g.$win, "keydown", function (e) {
					var t = e.which;
					d && d.hasClass("fr-uploading") && t == _e.KEYCODE.ESC && d.trigger("abortUpload")
				}), g.events.on("destroy", function () {
					d && d.hasClass("fr-uploading") && d.trigger("abortUpload")
				}), g.events.on("paste.before", oe), g.events.on("paste.beforeCleanup", le), g.events.on("paste.after", ne), g.events.on("html.set", A), g.events.on("html.inserted", A), A(), g.events.on("destroy", function () {
					o = []
				...
				}), g.opts.iframe && g.events.on("image.loaded", g.size.syncIframe), g.$wp && (E(), g.events.on("contentChanged", E)), g.events.$on(c(g.o_win), "orientationchange.image", function () {
					setTimeout(function () {
						d && T(d)
					}, 100)
			...
			showInsertPopup: function Ee() {
				...
				if (t || (t = X()), k(), !t.hasClass("fr-active"))
					if (g.popups.refresh("image.insert"), g.popups.setContainer("image.insert", g.$tb), e.isVisible()) {
			...
			showLayer: function Se(e) {
				...
				if (d || g.opts.toolbarInline) {
					if (d) {
						var r = we();
						Ce() && (r = r.find(".fr-img-wrap")), a = r.offset().top + r.outerHeight(), t = r.offset().left
					}
				} else {
			...
			insertByURL: function xe() {
				...
				if (0 < e.val().length) {
					...
					if (g.opts.imageUploadRemoteUrls && g.opts.imageCORSProxy && g.opts.imageUpload) {
						...
						a.onload = function () {
							200 == this.status ? G([new Blob([this.response], {
			...
			align: function $e(e) {
				var t = we();
				t.removeClass("fr-fir fr-fil"), !g.opts.htmlUntouched && g.opts.useClasses ? "left" == e ? t.addClass("fr-fil") : "right" == e && t.addClass("fr-fir") : me(t, he(), e), be(), S(), n(), g.selection.clear()
			},
			refreshAlign: function Ue(e) {
				d && e.find("> *").first().replaceWith(g.icon.create("image-align-" + ue()))
			},
			refreshAlignOnShow: function Ie(e, t) {
				d && t.find('.fr-command[data-param1="' + ue() + '"]').addClass("fr-active").attr("aria-selected", !0)
			},
			display: function Pe(e) {
				var t = we();
				t.removeClass("fr-dii fr-dib"), !g.opts.htmlUntouched && g.opts.useClasses ? "inline" == e ? t.addClass("fr-dii") : "block" == e && t.addClass("fr-dib") : me(t, e, ue()), be(), S(), n(), g.selection.clear()
			},
			refreshDisplayOnShow: function Oe(e, t) {
				d && t.find('.fr-command[data-param1="' + he() + '"]').addClass("fr-active").attr("aria-selected", !0)
			},
			replace: ve,
			back: function e() {
				d ? (g.events.disableBlur(), c(".fr-popup input:focus").blur(), T(d)) : (g.events.disableBlur(), g.selection.restore(), g.events.enableBlur(), g.popups.hide("image.insert"), g.toolbar.showInline())
			},
			get: ye,
			getEl: we,
			insert: L,
			...
			applyStyle: function ke(e, t, a) {
				...
				"object" == Le(t[e]) ? (i.removeAttr("style"), i.css(t[e].style)) : i.toggleClass(e), T(d)
			},
			showAltPopup: j,
			showSizePopup: J,
			setAlt: function Ne(e) {
				if (d) {
					...
					d.attr("alt", e || t.find("input").val() || ""), t.find("input:focus").blur(), T(d)
				}
			},
			...
			toggleCaption: function Te() {
				...
				if (d && !Ce()) {
					...
					e.wrap("<span " + (g.browser.mozilla ? "" : 'contenteditable="false"') + 'class="fr-img-caption ' + d.attr("class") + '" style="' + (g.opts.useClasses ? "" : e.attr("style")) + '" draggable="false"></span>'), e.wrap('<span class="fr-img-wrap"></span>'), d.after('<span class="fr-inner"' + (g.browser.mozilla ? "" : ' contenteditable="true"') + ">" + _e.START_MARKER + g.language.translate("Image Caption") + _e.END_MARKER + "</span>"), d.removeAttr("class").removeAttr("style").removeAttr("width"), d.parents(".fr-img-caption").css("width", i), pe(!0), g.selection.restore()
				} else e = we(), d.insertAfter(e), d.attr("class", e.attr("class").replace("fr-img-caption", "")).attr("style", e.attr("style")), e.remove(), T(d)
			},
			hasCaption: Ce,
			exitEdit: pe,
			edit: T
		...
		html: function () {
			...
			for (var a in t)
				if (t.hasOwnProperty(a)) {
					var i = t[a];
					"object" == Le(i) && (i = i.title), e += '<li role="presentation"><a class="fr-command" tabIndex="-1" role="option" data-cmd="imageStyle" data-param1="' + a + '">' + this.language.translate(i) + "</a></li>"
				}
			return e += "</ul>"
		},
		callback: function (e, t) {
